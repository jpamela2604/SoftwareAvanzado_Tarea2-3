...
app.post('/iniciar_viaje', (req, res) => {
  console.log('El cliente ' + req.body.ID + '  ubicado en: latitud ' + req.body.ID + ', ' + 'altitud ' + ' desea iniciar un viaje hacia: latitud ' + req.body.ID + ', ' + 'altitud ')
  var num = Math.random()
  if (num < 0.2) {
...
app.post('/piloto', (req, res) => {
  console.log('el viaje:' + req.body.viaje + ' con origen en: latitud ' + req.body.ID + ', ' + 'altitud ' + ' y destino: latitud ' + req.body.ID + ', ' + 'altitud busca un piloto')
  if (listaPilotosDisponibles.length === 0) {
    var obj = { status: false, mensaje: 'No hay pilotos disponibles' }
    var myJSON = JSON.parse(JSON.stringify(obj))
    res.send(myJSON)
  } else {
...
app.post('/rastreo', (req, res) => {
  ...
  if (num < 0.2) {
    ...
    var longitud = Math.floor(Math.random() * ((20) - (-10) + 1) + (-10)) + Math.random()
    var cadena = { status: true, latitud: latitud, longitud: longitud }
    var cadenaJson = JSON.parse(JSON.stringify(cadena))
    res.send(cadenaJson)
  }
})
